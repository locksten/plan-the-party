...
const tiles = [
  {
    title: "Rinkitės",
    text: "Vaišes, veiklas ir papuošimus",
    image: ITEM_ART_SOURCES["paper-airplane-challenge"],
    color: "bg-orange-soft",
  },
  {
    title: "Skaičiuokite",
    text: "Ar užteks pinigų ir porcijų?",
    image: ITEM_ART_SOURCES["mini-sandwiches"],
    color: "bg-blue-soft",
  },
  {
    title: "Prisitaikykite",
    text: "Netikėtumai keis jūsų planą",
    image: EVENT_ART_SOURCES.issiliejo,
    color: "bg-yellow",
  },
  {
    title: "Susitarkite",
    text: "Priimkite sprendimą kartu su klase",
    ...
    <section className="grid h-dvh grid-cols-[0.82fr_1.18fr]">
      <div className="relative flex flex-col justify-center overflow-hidden bg-coral px-[clamp(3rem,6vw,5.75rem)] pb-36 pt-10">
        ...
        <h1 className="relative m-0 text-[clamp(3.625rem,5.3vw,4.75rem)] font-black leading-[0.84] tracking-[-0.065em] text-cream [text-shadow:0.3125rem_0.3125rem_0_#17233f]">
          <span className="block whitespace-nowrap">Ką pasirinks</span>
          <span className="block whitespace-nowrap text-yellow">jūsų</span>
          <span className="block whitespace-nowrap">klasė?</span>
        </h1>
        <button
          ...
          <div className="game-scrollbar relative mt-7 flex max-h-[23.75rem] w-full max-w-[32.5rem] flex-col gap-3 overflow-y-auto pb-2 pr-3 pt-1">
            {missions.map((mission) => (
              <div className="flex gap-2" key={mission.id}>
                ...
                >
                  Tęsti su {mission.classLabel} klase
                </button>
                <button
                  ...
                  type="button"
                  aria-label={`Pašalinti ${mission.classLabel} klasės įrašą`}
                  title={`Pašalinti ${mission.classLabel} klasės įrašą`}
      ...
      <div className="grid grid-cols-2 grid-rows-2 gap-5 p-8 pr-12">
        {tiles.map((tile, index) => (
          <article key={tile.title} className={`relative isolate flex min-h-0 flex-col overflow-hidden rounded-[2.375rem] border-[0.25rem] border-navy ${tile.color} p-6 shadow-[0.4375rem_0.5rem_0_#17233f] ${index % 2 === 0 ? "-rotate-1" : "rotate-1"}`}>
            <h2 className="relative z-10 m-0 text-[clamp(2rem,3vw,3rem)] font-black leading-none">{tile.title}</h2>
            <p className="relative z-10 mt-2 text-xl font-extrabold">{tile.text}</p>
            <img
